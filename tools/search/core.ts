export type SearchDocument = { url: string; title: string; text: string };
...
export function searchSubstringIndex(index: SubstringIndex, input: string): SearchHit[] {
...
export function validateSubstringIndex(value: unknown): SubstringIndex {
  ...
  for (const item of value.documents) {
    ...
    documents.push({ url: item.url, title: item.title, text: item.text });
  }
